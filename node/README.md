...
// The QR code is provided as a data url, so you can render it inline on your page. `create_png_qrcode` also exists if you want a png.
...
// When the user logs in, once you validate their password, respond with the user_id as a hidden input field and ask them to enter their 2FA code.
if (simple2fa.check_2fa_code(user.otp_secret, <otp_code>)) {
    // Log in successful
} else {
